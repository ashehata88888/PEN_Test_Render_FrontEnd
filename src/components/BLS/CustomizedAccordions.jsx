...
export default function CustomizedAccordions({displayPassMainBtns,count,delCount,saveCount ,setCountCustAccorToFullW , setCountCustAccorToFullWC}) {
  const [expanded, setExpanded] = useState('panel99')
  const [displayAccordion, setDisplayAccordion] = useState(hTabs.callInfoAccordionhidden)
  ...
  console.log(" the new COUNT of Savging from customer to Customzed According is ",countSavCmrToCusAccor)
 ...
 let valContArr = [
  { contactPerson: '', mobileNumber: '', authority: '', speciality: '', position: '' }
]
  ...
  useEffect(() => {
    ...
    let contLength = val[val.length - 1]['contData'].length
      ...
      if(
        ...
        mNRegex.test(val[val.length - 1]['contData'][contLength - 1].mobileNumber) &&
      ...
      if(val[val.length - 1]['contData'][contLength - 1].contactPerson === '' ){
        ...
        divElement?.setAttribute("style", "background-color : red;")
      }else{  const divElement = elementCPRef?.current;
        divElement?.setAttribute("style", "background-color : none;")}

        
        let mobileValue = val[val.length - 1]['contData'][contLength - 1].mobileNumber
      if(mobileValue === '' ||
      !mNRegex.test(val[val.length - 1]['contData'][contLength - 1].mobileNumber)
      ){
        //  event.currentTarget.className = `${hTabs.dropDownPErr}`
        ...
        console.log("Contact Person Element is :",divElement); 
        divElement?.setAttribute("style", "background-color : red;")
           }else{  const divElement = elementMNRef?.current;
            divElement?.setAttribute("style", "background-color : none;")}
      ...
      if(val[val.length - 1]['contData'][contLength - 1].authority == 0 ){
        ...
        console.log("Contact Person Element is :",divElement); 
        divElement?.setAttribute("style", "background-color : red;")
           }else{  const divElement = elementARef?.current;
            divElement?.setAttribute("style", "background-color : none;")}
      ...
      if(val[val.length - 1]['contData'][contLength - 1].speciality == 0 ){
        ...
        console.log("Contact Person Element is :",divElement); 
        divElement?.setAttribute("style", "background-color : red;")
            }else{  const divElement = elementSRef?.current;
              divElement?.setAttribute("style", "background-color : none;")}
      ...
      if(val[val.length - 1]['contData'][contLength - 1].position == 0 ){
        ...
        console.log("Contact Person Element is :",divElement); 
        divElement?.setAttribute("style", "background-color : red;")
            }else{  const divElement = elementPRef?.current;
              divElement?.setAttribute("style", "background-color : none;")}
    ...
    val.flatMap(async (objP, index) => {
      ...
      const res = await fetch('https://pen-test-render-backend-ashehata8888.onrender.com/api/call_products', {
        ...
        objP.contData.map((objPC, indexC)=>{
          ...
          //  setPostCallContactData(postCallContactData.length !== 0 && postCallContactData)
          console.log("postCall contact Data in contatact loop are :", postCallContactData)
  ...
  const addContactBtn = (event , index) => {
    ...
    setIndexP(index)
         ...
         if(val[index]['contData'][contLength - 1].contactPerson !== '' &&
          ...
          xCount > 0 && xCount < 31 && val[index]['contData'].push(NewContactCallRow)
        }
      ...
      if(val[index]['contData'][contLength - 1].mobileNumber === '' ||
      !mNRegex.test(val[index]['contData'][contLength - 1].mobileNumber)
      ){
        //  event.currentTarget.className = `${hTabs.dropDownPErr}`
        const divElement = elementMNRef.current;
          ...
          useEffect(() => {
              ...
              if (IndexPD > 0 && xDCount > 0 && IndexContD < 20) {
                  ...
                  useEffect(() => {
                   ...
                   console.log("testing for validateCustomAccordion ...customAcorCount is ",customAcorCount)
...
<Fragment>
      <form
       onSubmit={()=>console.log('CustomizedAccordion form was submitted')}
        ...
        >
          ...
          <div className={hTabs.productBox}>
            ...
            <select
              ...
              className={errSupplier ? hTabs.dropDownPErr : hTabs.productBoxSelect}
             id={index}
          ...
          <label className={hTabs.CallLable}>Customer Information</label>
             ...
             <div>
                { xCount > 0 &&  val[index]['contData'].map((dataC, indexC) => {
