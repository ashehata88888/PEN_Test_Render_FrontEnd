...
// "https://backend-ahmed-shehata-crm.onrender.com/api/bus"
...
function Activity() {
  ...
  const fetchData = () => {
    return (
      fetch('https://backend-ahmed-shehata-crm.onrender.com/api/bus', {
        method: 'GET',
        headers: new Headers({
  ...
  return (
    <main>
      ...
      <select name="cars" id="cars">
        {user &&
          user.length > 0 &&
          user.map((userObj, index) => <option value={userObj}>{userObj}</option>)}
      </select>
    </main>
  )
}
