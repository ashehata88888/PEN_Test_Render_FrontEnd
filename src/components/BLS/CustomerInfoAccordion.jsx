...
bgsave()
    ...
    export default function customerInfoAccordion({ displayPass, setActivityId, setAccountIdPass, saveCount, setCustomAcorCount, savCustomerCont, setFinalCustomAccordCount, setCountSavCmrToCusAccor}) {
...
// 16370409   
  ...
  activityTypeValue === "5" )
          ...
          if((accountId === 0 || accountId == "" || accountNameRef.current.value == "") && 
            ( activityTypeValue === "1" ||
  activityTypeValue === "2" ||
  activityTypeValue === "3" ||
  activityTypeValue === "4" ||
        ...
        switch(checked) {
          case false :
            ...
            if (
              ...
              console.log(`check ${checked} is Save Button validation false from 6 to 13 Pressed and called save Function`)
            //  validateCustomAccordionCount()
              ...
              purchaseMethodId !== 0)
             {
              validateCustomAccordionCount()
              // saveBtnHandler()
               console.log(`check ${checked} Save Button with checkbox validation false from 1 to 5 Pressed and called save Function`)
            }
          ...
          case true:
            ...
            if (
              ...
              console.log(`check ${checked} Save Button validation True (startDate !== null  && endDate !== null) from 6 to 13 Pressed and called save Function`)
              // validateCustomAccordionCount()
              saveBtnHandler()
              ...
              purchaseMethodId !== 0)
             {
              validateCustomAccordionCount()
              // saveBtnHandler()
               console.log(`check ${checked} Save Button with checkbox validation True (startDate !== null  && endDate !== null) from 1 to 5 Pressed and called save Function`)
            }
            break;
          default:
  ...
  setCustomAcorCount(current => current + 1)
      ...
      const commentTxtOnChange = (e)=>{
        ...
        if(e.target.value.length === 250){
          setCommetAlret("You've reached the max length of the Text area (250 Characters)")
        }
...
useEffect(() => {
    ...
    async function fetchData() {
      // You can await here
        ...
        console.log("Final Save Function for Customer Info was called and save count indide the useEffect of Saving Data is ......:", savCustomerCont)
        await saveBtnHandler().then(()=>setFinalCustomAccordCount(current=> current + 1 ))
        

      // ...
    }
    if(savCustomerCont > 0){
      console.log("Save Btn Handler of Customer info Called and then Set Fial Custom Accordion Count Called")
    fetchData();
  }
      ...
      return (
        saveDone ? <div className={hTabs.backgroundSave}>
        ...
        >
            <span className={hTabs.errSpan}>{alret}{saveSuccessMsg}</span>
          <div
          ...
          >
            ...
            <select
              ...
              className={errActivitySelectType ? hTabs.dropDownErr : hTabs.dropDownList}
              id="activityType"
              name="activityType"
            ...
            {/* <div
              className={displayAccountsData}
               > */}
            <label className={`${hTabs.labels} ${displayAccountsData}`}>Account Type</label>
            <select
              // {...displayAccountsData === hTabs.displayActivityData ? "required" : console.log(displayAccountsData) }
              // ref={account_type_id}    
              className={`${errAccountType ? hTabs.dropDownErr : hTabs.dropDownList} ${displayAccountsData}`}
              id="activityType"
              name="activityType"
            ...
            <label className={`${hTabs.labels} ${displayAccountsData}`}>Account Name</label>
          
            <input 
            className={`${errAccountName ? hTabs.dropDownErr : hTabs.inputNameList} ${displayAccountsData}`}
            type="text" value={accountNameValue} 
    ...
    <div className={`${hTabs.accountNamesdropDownList}
          ...
          {accountName.filter((item, i) => {
            ...
            .map((obj,index)=>{
              ...
              // <div className={hTabs.dropdownDiv}>
               ...
               <div
                ...
                onClick={(e) => onClickElementFun(e,obj.account_name,obj.id,index,mainIndex) }
            ...
            <label id="purchaseLabel" className={`${hTabs.labels} ${displayAccountsData}`}>
              Purchase Method
            </label>
            <select
              //  {...displayAccountsData === hTabs.displayActivityData ? required : console.log(displayAccountsData) }
              ...
              id="purchaseMethod"
              name="purchaseMethod"
              className={`${errpurchaseMethod ? hTabs.dropDownErr : hTabs.dropDownList} ${displayAccountsData}`}
